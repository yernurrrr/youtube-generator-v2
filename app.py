...
# ... (Оставьте весь код с импортами и функцией generate_titles без изменений)

# --- НОВАЯ КОНСТАНТА ДЛЯ ЛИМИТА ---
FREE_LIMIT = 5
...
if st.button("Сгенерировать 🚀"):
    ...
    if st.session_state.count >= FREE_LIMIT:
        ...
        with st.spinner("Генерация... Это может занять несколько секунд."):
            ...
            st.experimental_rerun() # Перезапускаем, чтобы обновить счетчик сразу
